...
const index = randomInt(0, arr.length); // random index to be used
/**
...
document.getElementById("quote").innerHTML =
  JSON.parse(JSON.stringify(getQuote())).text;
// api make same page able to play a random song from apple api
const getSong = () => {
  const xhr = new XMLHttpRequest();
  xhr.open("GET", "https://itunes.apple.com/search?term=music&limit=1", true);
  xhr.onload = () => {
...
// make same page display a random gift  from api.gip
const getGift = () => {
  const xhr = new XMLHttpRequest();
  xhr.open(
    "GET",
    "https://api.giphy.com/v1/gifs/random?api_key=TIt7PMgCV2JANI3z0r34B3LZCiwxz8Gv&tag=hacker&rating=g",
    true,
  );
  xhr.onload = () => {
    if (xhr.readyState === 4 && xhr.status === 200) {
      const data = JSON.parse(xhr.responseText);
      const gif = data.data.images.fixed_width_still.url;
      const img = document.getElementById("gift");
      console.log("stas", gif);
      img.src = gif;
    }
  };
  xhr.send();
